...
export function Checkbox(attrs: Attrs<HTMLButtonElement> & CheckboxAttributes, children: Renderable[]) {
  ...
  let classes = {
    ...
    [css_checkbox_off]: o_model.tf(m => m == false),
  ...
  return <button disabled={attrs.disabled} class={[css_checkbox_label, control.css_control, classes]}>
    ...
    {If(children.length > 0, () => <$>
      {' '}
      <span class={[css_checkbox_content]}>{children}</span>
    </$>)}
...
export const css_checkbox_label = style('label', CSS.border(T.tint14).background(T.bg).cursorPointer)
...
export const css_checkbox_content = style('content')
